...
handoffs:
  - label: Create BRD First
    agent: productkit.brd
    prompt: Start with Business Requirements Document for strategic context
  - label: Plan Technical Implementation
    agent: productkit.plan
    prompt: Create technical design document and implementation plan
  - label: Break into Tasks
    agent: productkit.tasks
    prompt: Break this PRD into actionable development tasks
---
